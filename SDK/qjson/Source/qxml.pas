...
try
  ...
  on E: Exception do
    ...
    raise Exception.CreateFmt('%s'#13#10'位置:第%d行 第%d列'#13#10'行内容：%s',
      [E.Message, ARow, ACol, DecodeLineW(pl)]);
    end;
end;
end;
    ...
    case AVal.Kind of
      ...
      tkFloat:
        ...
        if (AVal.TypeInfo = TypeInfo(TDateTime)) or
          (AVal.TypeInfo = TypeInfo(TTime)) or (AVal.TypeInfo = TypeInfo(TDate))
        then
          begin
        ...
        case AFields[J].FieldType.TypeKind of
          ...
          tkEnumeration:
            ...
            if GetTypeData(AFields[J].FieldType.Handle)
              ...
              begin
              Attrs.Add('type', 'bool');
              Attrs.Add('value', BoolToStr(AFields[J].GetValue(ASource)
                .AsBoolean, True));
              end
            else
              begin
          ...
          tkFloat:
            ...
            if (AFields[J].FieldType.Handle = TypeInfo(TDateTime)) or
              ...
              Attrs.Add('value', FloatToStrF(AFields[J].GetValue(ASource)
                .AsExtended, ffGeneral, 15, 0));
              end;
            end;
          tkInt64:
            begin
...
function TQXMLNode.HasChild(ANamePath: QStringW;
  var AChild: TQXMLNode): Boolean;
begin
...
var
  ...
  if StartWithW(ps, '<?xml', True) then
    ...
    begin
    // DTD,忽略
    if StartWithW(ps, '<!DOCTYPE', False) or StartWithW(ps, '<!ELEMENT', False)
      or StartWithW(ps, '<!ATTLIST', False) then
      begin
      AMS.Size := 0;
      ParseUCS2;
      end
  ...
  if CompareMem(ps, @XMLDeclare[0], 5) then
    ...
    begin
    // DTD,忽略
    if CompareMem(ps, @DTDDocType[0], 9) or CompareMem(ps, @DTDElement[0], 9) or
...
if Assigned(ANode.Parent) then
  TQHashedXMLNode(ANode.Parent).FHashTable.Add(Pointer(ANode.Parent.Count - 1),
    ANode.FNameHash);
end;
