...
1、调用TQAndroidShell.Create方法来初始化一个实例
AShell:=TQAndroidShell.Create;
...
type
  ...
  public
    ...
    function Execute(const ACmdline:QStringW;ATimeout:Cardinal=INFINITE):QStringW;
...
function TQAndroidShell.Execute(const ACmdline: QStringW;ATimeout:Cardinal): QStringW;
var
  AProcess:JProcess;
begin
SetLength(Result,0);
if InRoot then
  ...
  if Assigned(AProcess) then
    Result:=InternalReadReply(AProcess,INFINITE);
  end;
end;
...
function TQAndroidShell.InternalReadReply(AProcess: JObject;
  ATimeout: Cardinal): QStringW;
var
