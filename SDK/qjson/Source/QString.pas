...
/// <returns>返回过滤后的结果</returns>
function FilterCharW(const S: QStringW; AOnValidate: TQFilterCharEvent;ATag:Pointer=nil)
  : QStringW; overload;
{$IFDEF UNICODE}
/// <summary>过滤掉字符串内容中不需要的字符</summary>
/// <param name="S">要过滤的字符串</param>
...
/// <param name="ATag">用户自定义的附加参数，会传递给AOnValidate事件</param>
/// <returns>返回过滤后的结果</returns>
function FilterCharW(const S: QStringW; AOnValidate: TQFilterCharEventA;ATag:Pointer=nil)
  : QStringW; overload;
{$ENDIF}
...
function FilterCharW(const S: QStringW; AOnValidate: TQFilterCharEvent;ATag:Pointer)
  : QStringW; overload;
var
  ps, pd, pds: PQCharW;
  l, I: Integer;
...
function FilterCharW(const S: QStringW; AOnValidate: TQFilterCharEventA;ATag:Pointer)
  : QStringW; overload;
var
  ps, pd, pds: PQCharW;
  l, I: Integer;
...
var
  AFreeEvent: TQPtrFreeEvent;
begin
TMethod(AFreeEvent).Data := nil;
TMethod(AFreeEvent).Code := @AOnFree;
Result := Bind(AData, AFreeEvent);
end;
...
if Assigned(FObject) then
  ...
  if Assigned(FOnFree) then
    ...
    if TMethod(FOnFree).Data = nil then
      TQPtrFreeEventG(FOnFree)(FObject)
    else
      FOnFree(FObject);
    end
{$IFDEF UNICODE}
  else if Assigned(FOnFreeA) then
    FOnFreeA(FObject)
{$ENDIF}
  else
    FreeAndNil(FObject);
