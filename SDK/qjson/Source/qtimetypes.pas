...
uses classes, qstring, sysutils, dateutils, SqlTimSt;
...
var
  L: Int64;
begin
L := Integer(FData shr 40);
if (L and $800000) <> 0 then
  L := -L;
Y := L div Month1Year;
if L < 0 then
  begin
...
if (MST > 549755813887) or (D < -549755813888) then // 低40位表示范围
  raise Exception.Create(SIntervalOutOfRange);
FData := MT shl 40 + MST;
end;
    ...
    if (N <> 0) or (S <> 0) or (MS <> 0) then
      ...
      if (S <> 0) or (MS <> 0) then
        ...
        if MS <> 0 then
          ...
          Result := Result + '.';
          if (MS mod 100) = 0 then
            Result := Result + IntToStr(MS div 100) + ''' ' + SF + ' TO SECOND('
              + IntToStr(Length(SS)) + ',1)'
          else if (MS mod 10) = 0 then
            Result := Result + IntToStr(MS div 10) + ''' ' + SF + ' TO SECOND('
              + IntToStr(Length(SS)) + ',2)'
          else
...
if (H <> 0) or (N <> 0) or (S <> 0) or (MS <> 0) then
  begin
  Result := Result + ' ' + IntToStr(H) + ':' + IntToStr(N) + ':' + IntToStr(S);
  if MS <> 0 then
    begin
    Result := Result + '.';
    if (MS mod 100) = 0 then
  ...
  if ParseInt(P, V) <> 0 then
    ...
    if (P^ = 'y') or (P^ = 'Y') then
      ...
      if (P^ = #0) or IsSpaceW(P) or CheckTrailer('min', 3) or
        ...
        M := V
      else if StartWithW(P, 'ms', True) or CheckTrailer('millisecond', 11) then
        MS := V
...
if (Y <> 0) or (M <> 0) or (D <> 0) then // 有日期
  ...
  if D > MonthDays[IsLeapYear(Y)][M] then
    raise Exception.CreateFmt(SBadTimeValue, [Y, M, D, H, N, S, MS]);
  if (Y=1582) and (M=10) and (D>=5) and (D<=14) then //1582-10-5~1582-10-14在阳历中是无效的日期
    raise Exception.CreateFmt(SBadTimeValue, [Y, M, D, H, N, S, MS]);
  end
...
Data := (Data and $FFFFFFE0FFFFFFFF) or ((Int64(Value) shl 32) and $1F00000000);
end;
