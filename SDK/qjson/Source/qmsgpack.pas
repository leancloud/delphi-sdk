...
function TQMsgPack.CopyIf(const ATag: Pointer; AFilter: TQMsgPackFilterEventA)
  ...
  for I := 0 to AParentSource.Count - 1 do
    ...
    if Accept then
      begin
      AChildDest := AParentDest.InternalAdd(AChildSource.FKey,AChildSource.KeyType,
...
function TQMsgPack.EncodeAsString(AStrictJson: Boolean): QStringW;
  ...
  case AItem.DataType of
    ...
    mptBoolean:
      Result := BoolToStr(AItem.AsBoolean, True);
    mptFloat:
        ...
        case APropList[J].PropType^.Kind of
          tkClass:
            ...
            if AChildObj is TStrings then
              ...
              AddCollection(Add(AName, mptArray), AChildObj as TCollection)
            else
...
if KeyType = mptBoolean then
  ...
  raise Exception.Create(Format(SBadConvert, [MsgPackTypeName[Integer(KeyType)],
    'Boolean']));
end;
...
case FKeyType of
  ...
  mptDateTime:
    begin
    if SameValue(PDateTime(PQCharW(FKey))^ - Trunc(PDateTime(PQCharW(FKey))^), 0)
    then
      Result := FormatDateTime(MsgPackDateFormat, PDateTime(PQCharW(FKey))^)
    else
      begin
      if Trunc(PDateTime(PQCharW(FKey))^) = 0 then
        Result := FormatDateTime(MsgPackTimeFormat, PDateTime(PQCharW(FKey))^)
      else
...
repeat
  if Assigned(AParent) and AParent.IsArray then
    ...
    Result := QMsgPackPathDelimiter + AItem.Name + Result
  else
    Result := QMsgPackPathDelimiter + AItem.Name + Result;
  AItem := AParent;
  ...
  if Length(AItem.FKey) = l then
    ...
    if MsgPackCaseSensitive then
      ...
      if AItem.FKeyHash = 0 then
        AItem.FKeyHash := HashOf(PQCharW(AItem.FKey), l);
    ...
    else if StartWithW(PQCharW(AItem.Name), PQCharW(AName), True) then
      begin
      Result := I;
      Break;
      end;
    ...
    for I := 0 to C - 1 do
      ...
      case AChild.FKeyType of
        mptUnknown:
          WriteNull;
        mptInteger:
          WriteInt(AChild.KeyAsInt64);
        mptNull:
          WriteNull;
        mptBoolean:
          WriteBool(AChild.KeyAsBoolean);
        mptSingle:
          WriteSingle(AChild.KeyAsSingle);
        mptFloat:
          WriteFloat(AChild.KeyAsFloat);
        mptString:
          WriteString(AChild.KeyAsString);
        mptBinary:
          WriteBinary(@AChild.FKey[0], Length(AChild.FKey));
        mptDateTime:
          WriteFloat(AChild.KeyAsDateTime);
      end;
...
Result.FKeyHash := HashOf(PQCharW(AName), Length(AName) shl 1);
FHashTable.Add(Pointer(Count - 1), Result.FKeyHash);
end;
...
procedure TQHashedMsgPack.DoNodeNameChanged(ANode: TQMsgPack);
  ...
  if AHash <> ANode.FKeyHash then
    begin
    AList := FHashTable.FindFirst(ANode.FKeyHash);
    while AList <> nil do
      begin
      ...
      if AItem = ANode then
        begin
        FHashTable.ChangeHash(Pointer(AIndex), ANode.FKeyHash, AHash);
...
for I := 0 to Count - 1 do
  ...
  if Length(AMsgPack.FKey) > 0 then
    ...
    FHashTable.Add(Pointer(I), AMsgPack.FKeyHash);
    end;
...
if Length(ANewItem.FKey) > 0 then
  FHashTable.Add(Pointer(AIndex), ANewItem.FKeyHash);
end;
